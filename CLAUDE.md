...
| FundTransfers | Funds Received | CASH_IN | Inbound wire — sole cash record (#28) |
...
28. **`Funds Received` credits cash; `Current Year Contribution` does NOT**: both are symbol-less external-cash rows but split two ways. `FundTransfers / Funds Received → CASH_IN` — it is the **sole** record of a wire (no `990156937` sweep Deposit mirrors it), so `reconstruct_cash` credits it like any `symbol=None` row. `FundReceipts / Current Year Contribution → CONTRIBUTION_INFO` — kept in Transactions for the record but **excluded from cash math** (`_CASH_EXCLUDED_TYPES` in `cash.py`), because the same dollars already arrive as an `IIAXX` Deposit and counting both **double-counts**. Guards: `test_funds_received_wire_credits_cash`, `test_contribution_info_excluded_no_double_count`, `test_funds_received_is_cash_in`, `test_current_year_contribution_is_recorded_not_cash`.
