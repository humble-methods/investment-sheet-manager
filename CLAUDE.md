...
| SecurityTransactions | Stock Dividend Due Bill | SPLIT | `$0`/share split leg (see #27) |
| SecurityTransactions | Dividend | SPLIT | ONLY when `amount == 0` + share qty (split delivery); else UNKNOWN. See #27 |
| FundTransfers | Funds Received | CASH_TRANSFER_IN | Inbound wire — sole cash record; **Amount positive-for-inflow** (#28) |
| FundReceipts | Current Year Contribution | CONTRIBUTION_INFO | Recorded only, excluded from cash math (#28) |
...
23. **`Performance Compare` self-heals its grid but preserves the user's picks**: the ONLY user state is **row 1, cols B..** (the picked symbols in the dropdown slots); the column-A labels and every card value (a *formula* into the `Performance`/`Performance By Year` tabs; year rows are `YEAR(TODAY())`-relative) are deterministic. So `write_performance_compare` **creates the tab + full grid + dropdowns when absent**, and when the tab already exists it **rewrites only the deterministic cells** — `A1` label (1×1, so it can't touch B1..) then rows 2+ at `A2` — **never** row-1 slot values and **never** re-applying data validation on the heal path (re-applying would revert a slot the user manually converted to a multi-select "chip" dropdown, which the Sheets API can't recreate — it only writes single-select `ONE_OF_RANGE` via `sh.batch_update`). This fixes the "dropdowns present, no labels/formulas" state an older create-only scaffold left behind, without wiping selections. Do NOT go back to a blind clear-and-rewrite (would nuke picks) NOR to a pure no-op-when-present (a broken grid never heals). Guards: `test_write_performance_compare_heals_grid_preserving_picks_when_present`, `test_write_performance_compare_sets_dropdown_validation`, `test_write_performance_compare_scaffolds_when_absent`.
...
26. **Unmapped `(Type, Description 1)` combos are silently dropped from holdings**: the activity parser tags any combo not in `TX_TYPE_MAP` as `tx_type="UNKNOWN"` (logged, not raised) and `build_lots` ignores everything but BUY/INIT_BUY/SELL — so the row is kept in Transactions but has **zero effect on share counts or cash**. This is fine for true non-events, but real share/cash-moving types currently fall through this crack: **`SecurityTransactions / Stock Dividend Due Bill`** and **`SecurityTransactions / Dividend`** with a share quantity (stock dividends / splits — e.g. KLAC +306 on 34 sh ≈ 10-for-1, VGT +70 on 10 ≈ 8-for-1, in the 04–06/2026 file), plus **`FundTransfers / Funds Received`** (inbound cash) and **`FundReceipts / Current Year Contribution`** (Roth contribution). Effect if unhandled: holdings **understate** post-split quantities (and it's a latent oversell next period if those shares are sold), and the Cash tab misses transfers. These four combos are **now handled** — splits/stock dividends via a new `SPLIT` type (#27), cash transfers/contributions via `CASH_IN`/`CONTRIBUTION_INFO` (#28); the general "unmapped combo → UNKNOWN → silently dropped" behavior still holds for any combo NOT in `TX_TYPE_MAP`.

27. **Splits / stock dividends scale existing lots in place (never add `$0` lots)**: the parser maps `SecurityTransactions / {Stock Dividend Due Bill, Dividend}` to a new **`SPLIT`** type **only when `amount == 0` AND a share quantity is present** — a real cash dividend (nonzero amount, no quantity) stays `DIVIDEND`. A split surfaces as several `$0`/share legs (`+N` due bill, `−N` reversal, `+N` delivery) that **NET** to the true share change; `build_lots` **pre-aggregates the net delta per `(account, symbol)`**, applies **one** scaling at the first leg (`qty ×ratio`, `unit_cost ÷ratio`, **acquisition date + total cost basis unchanged**), and skips later legs. Adding `$0`-cost shares at the event date instead would inject a phantom present-day acquisition and **wreck XIRR** — explicitly rejected. `SPLIT` sorts **after** same-day BUY/INIT_BUY (`_TYPE_ORDER`) so the ratio sees the full pre-split position; the broker `HOLDING N` base (parsed via `parse_holding_base`) cross-checks running qty and **logs a warning** on divergence (signals a missed earlier event). First case VGT ×8. Guards: `test_split_scales_lots_in_place`, `test_split_after_same_day_buy_sees_full_position`, `test_split_then_sell_no_oversell`, `test_split_injects_no_cashflow`, `test_split_legs_classified_as_split`, `test_cash_dividend_not_misclassified_as_split`.

28. **`Funds Received` credits cash; `Current Year Contribution` does NOT**: both are symbol-less external-cash rows but split two ways. `FundTransfers / Funds Received → CASH_TRANSFER_IN` — it is the **sole** record of a wire (no `990156937` sweep Deposit mirrors it). `FundReceipts / Current Year Contribution → CONTRIBUTION_INFO` — kept in Transactions for the record but **excluded from cash math** (`_CASH_EXCLUDED_TYPES` in `cash.py`), because the same dollars already arrive as an `IIAXX` Deposit and counting both **double-counts**. Guards: `test_funds_received_wire_credits_cash`, `test_contribution_info_excluded_no_double_count`, `test_funds_received_is_cash_transfer_in`, `test_current_year_contribution_is_recorded_not_cash`.

29. **Two OPPOSITE Amount-sign conventions coexist in `reconstruct_cash`**: the safe cash model replays only `symbol is None` rows, but their `Amount` is signed two incompatible ways. Sweep `Deposit`/`Withdrawal` (and `IIAXX` frac buy/sell) use Merrill's **parens-for-inflow** ledger convention — a Deposit ships `(19.00)` → parsed `-19` → credited via `delta = -amount`. But **`Bank Interest` (INTEREST) and `Funds Received` wires (CASH_TRANSFER_IN) ship the credit POSITIVE** (`"2.00"`, `"30,000.00"` — no parens), the investor's perspective. A uniform `delta = -amount` therefore books those inflows **backwards** — a real `$30,000` wire landed as **−$30,000**, producing impossible negative cash (observed: accounts `73S-17D17`, `79Z-10248` each ~`$60k` too low = missing `+30k` credit **plus** an erroneous `−30k` debit). Fix: `_CREDIT_CONVENTION_TYPES = {"INTEREST", "CASH_TRANSFER_IN"}` add `+amount`; everything else keeps `-amount`. `DIVIDEND`/`ADR_FEE`/`TAX_WITHHOLDING` are unaffected (they carry a symbol → skipped). Guards: `test_bank_interest_credits_cash`, `test_funds_received_wire_credits_cash`, `test_funds_received_not_subtracted_like_sweep_deposit`.

30. **Set-up-once tabs must self-heal a pre-existing EMPTY state, not just create-on-absent**: the two human-input tabs (`Watchlist`, `Performance Compare`) were written only when *created*, so a tab that already exists but is **blank** — from an older code version, a partial run, or a hand-made empty tab — never received its scaffold and looked broken (`Watchlist` had **no `Symbol` header**; `Performance Compare` showed **dropdowns but no labels/formulas**). `_ensure_tab` writes headers only on creation, so it does NOT fix a pre-existing empty tab. Fix pattern (applied to both): detect the fully-blank / grid-missing tab and lay down just the header/deterministic scaffold, **never touching existing user rows or dropdown picks**. `load_watchlist_symbols` writes `WATCHLIST_HEADERS` when the tab has no non-empty cell; `write_performance_compare` rewrites the label+formula grid when present (see #23). Guards: `test_load_watchlist_symbols_heals_headerless_empty_tab`, `test_write_performance_compare_heals_grid_preserving_picks_when_present`.
